...
void RBT<T>::insert(const T& toInsert, RBTNode<T>*& point, RBTNode<T>* parent) {
    ...
    if (getColor(point) == BLACK && getColor(point->left) == RED && getColor(point->right) == RED) {
        point->color = RED; point->left->color = BLACK; point->right->color = BLACK;
    }
    else if (getColor(point->right) == RED && getColor(point->right->right) == RED) {
        singleCCR(point);
    }
    else if (getColor(point->left) == RED && getColor(point->left->left) == RED) {
        singleCR(point);
    }
    else if (getColor(point->left) == RED && getColor(point->left->right) == RED) {
        doubleCR(point);
    }
    else if (getColor(point->right) == RED && getColor(point->right->left) == RED) {
        doubleCCR(point);
    }
}
